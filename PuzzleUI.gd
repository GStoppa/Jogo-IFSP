...
func gerar_nova_equacao():
	...
	if randf() < 0.75:
		...
		if randf() < 0.5:
			...
			equation_label.text = "%d(x %s) = %d" % [a, texto_b, c]
			respostas_corretas = [str(x)]

	else:
		var tipo_segundo_grau = randi_range(0, 2)
		match tipo_segundo_grau:
			0:
				...
				var texto_b = "+ %dx" % b
				if b < 0: texto_b = "- %dx" % abs(b)
				
				var texto_c = "+ %d" % c
				if c < 0: texto_c = "- %d" % abs(c)
			...
			1:
				var a = randi_range(1, 3)
				var r = randi_range(2, 5)

				var c = -a * (r * r)
				
				var texto_c = "- %d" % abs(c)
				
				equation_label.text = "%dx² %s = 0" % [a, texto_c]
				respostas_corretas = [str(r), str(-r)]
			...
			2:
				...
				equation_label.text = "%dx² %s = 0" % [a, texto_b]
				respostas_corretas = ["0", str(r)]
